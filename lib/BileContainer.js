...
bilecontainer.set = function () {
    bilecontainer.cleanIcons.call(this);
    this._warns = {};
    this._errors = {};
...
bilecontainer.add = function (path, importInfo) {
    if (!bilecontainer.isIgnore.call(this, path)) {
        ...
        for (var i = 0; i < this._iconpath.length; i++) {
            if (path.indexOf(this._iconpath[i]) === 0) {
                isicon = true;
                break;
            }
        }
        ...
        if (!isicon) {
            try {
                ...
                bilecontainer.doIcons.call(this, bile);
            } catch (e) {
                console.error(e);
            }
        } else {
            var a = bile.getPacketShortName();
            var b = a.split(".");
            b.pop();
            var c = b.join(".") + ".icon";
            var r = this.getBile(c, "packet");
            if (r) {
                var d = r.getBasePath() + b.join("/");
                r._content = util.icon.getIconScript(r.getPacketShortName(), d);
                r._ismake = false;
                r._isoutput = false;
            }
        }
...
bilecontainer.remove = function (path) {
    if (!bilecontainer.isIgnore.call(this, path)) {
        ...
        if (!isicon) {
            ...
            var a = util.path.getPacketName(this._path, path);
            var b = a.split(".");
            b.pop();
            var c = b.join(".") + ".icon";
            var r = this.getBile(c, "packet");
...
bilecontainer.doIcons = function (bile) {
    if (bile instanceof packetbile) {
        var icon = bile.getIcon();
        if (icon && icon.length > 0) {
            for (var i = 0; i < icon.length; i++) {
                ...
                if (this._iconpath.indexOf(_a) === -1) {
                    this._iconpath.push(_a);
                }
                var path = _a + "/icon.js";
                var content = util.icon.getIconScript(packet, _a);
                var _bile = new packetbile(this, path, content, bile.getImportInfo());
...
bilecontainer.cleanIcons = function () {
    var _delete = [];
    for (var i in this._registry) {
        var _a = this._registry[i];
        for (var t in _a) {
            var _b = _a[t];
            for (var m = 0; m < this._iconpath.length; m++) {
                if (_b._path.indexOf(this._iconpath[m]) === 0 && _b._path !== this._iconpath[m] + "/icon.js") {
                    _delete.push(i);
                }
            }
        }
    ...
    for (var i = 0; i < _delete.length; i++) {
        delete this._registry[_delete[i]];
    }
};
