...
bilecontainer.set = function () {
    bilecontainer.doIcons.call(this);
    this._warns = {};
    this._errors = {};
    var sourceMap = this._builder.getSourceMap();
    var _map = [];
...
bilecontainer.add = function (path, importInfo) {
    if (!bilecontainer.isIgnore.call(this, path)) {
        ...
        bilecontainer.checkIconEdit.call(this, path);
    }
};
...
bilecontainer.doIcons = function () {
    for (var i in this._registry) {
        for (var t in this._registry[i]) {
            var bile = this._registry[i][t];
            if (bile instanceof packetbile && bile.getIcon().length > 0) {
                var icons = bile.getIcon();
                for (var p = 0; p < icons.length; p++) {
                    var a = icons[p], packet = a + ".icon";
                    if (this._iconpath.indexOf(a) === -1) {
                        this._iconpath.push(a);
                    }
                    if(bile._require.indexOf(packet)===-1) {
                        bile._require.push(packet);
                    }
                    var _has=false;
                    for(var etnp in bile._depends){
                        if(bile._depends[etnp].value===packet){
                            _has=true;
                            break;
                        }
                    }
                    if(!_has) {
                        bile._depends.push({type: "packet", value: packet});
                    }
                    if (!this._registry[packet]) {
                        var _a = bile.getBasePath() + a.replace(/\./g, "/");
                        var path = _a + "/icon.js";
                        var _bile = new packetbile(this, path, "", bile.getImportInfo());
                        bilecontainer.setIconsContent.call(this, _bile);
                        var sn = _bile.getPacketShortName(), suffix = _bile.getSuffix();
                        if (!this._registry[sn]) {
                            this._registry[sn] = {};
                        }
                        this._registry[sn][suffix] = _bile;
                    } else {
                        for (var e in this._registry[packet]) {
                            if (!this._registry[packet][e]._isoutput) {
                                bilecontainer.setIconsContent.call(this, this._registry[packet][e]);
                            }
                        }
                    }
                }
...
bilecontainer.checkIconEdit = function (path) {
    var packetName = util.path.getPacketName(this._path, path);
    for (var i = 0; i < this._iconpath.length; i++) {
        if (packetName.indexOf(this._iconpath[i] + ".") === 0) {
            var biles = this._registry[this._iconpath[i] + ".icon"];
            if (biles) {
                for (var q in biles) {
                    biles[q]._ismake = false;
                    biles[q]._isoutput = false;
                }
            }
        }
    }
};
bilecontainer.setIconsContent = function (bile) {
    if (bile instanceof packetbile) {
        var packet = bile.getPacketShortName();
        var a = packet.split(".");
        if (a.length > 1) {
            a.pop();
        }
        ...
        var contents = [];
        for (var i in this._registry) {
            if (i !== (b + "icon") && i.indexOf(b) === 0) {
                for (var t in this._registry[i]) {
                    var _bile = this._registry[i][t];
                    _bile._ismake = true;
                    _bile._isoutput = true;
                    contents.push(_bile.getContent());
                }
            }
        }
        bile.setContent(util.icon.getIconScript(packet, contents));
        bile._ismake = false;
        bile._isoutput = false;
    }
};
...
module.exports = function (option, builder) {
    return new bilecontainer(option, builder);
};