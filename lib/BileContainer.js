...
var bilecontainer=function(option,builder){
    ...
    if(topolr.file(this._option.importPath).isExists()){
        topolr.file(this._option.importPath).scan(function (_path, isfile) {
            if (isfile) {
                bilecontainer.add.call(ths,_path,true);
            }
        });
    }
    if(topolr.file(this._option.basePath).isExists()){
        topolr.file(this._option.basePath).scan(function (_path, isfile) {
            if (isfile) {
                bilecontainer.add.call(ths,_path,false);
            }
        });
    }else{
...
bilecontainer.create=function (path,isimport) {
    var bile=null;
    ...
    if(!isbinaryfile.sync(_path)){
        ...
        if(ispacket){
            bile=new packetbile(this,_path,content,isimport);
        }else{
            bile=new textbile(this,_path,content,isimport);
        }
    }else{
        bile=new binarybile(this,_path,isimport);
    }
    return bile;
};
bilecontainer.add=function (path,isimport) {
    if(!bilecontainer.isIgnore.call(this,path)){
        try{
            var bile=bilecontainer.create.call(this,path,isimport);
...
bilecontainer.prototype.getImportPath=function () {
    return this._option.importPath;
};
...
bilecontainer.prototype.getBasicInfo=function () {
    ...
    return {
        total:r,
        js:this._sourceMap.js.length,
        css:this._sourceMap.css.length,
        template:this._sourceMap.template.length,
        packet:this._sourceMap.packet.length,
        text:this._sourceMap.text.length,
        image:this._sourceMap.image.length,
        json:this._sourceMap.json.length,
        html:this._sourceMap.html.length
    };
};
