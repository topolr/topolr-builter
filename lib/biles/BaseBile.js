...
var basebile=function(container,path){
	this._path = path;
    ...
    if(container) {
        ...
        this._packetName = util.path.getPacketName(container.getBasePath(), path);
        this._url = path.substring(container.getBasePath().length);
...
basebile.prototype.getShortPath=function () {
    return this._path.substring(this.getBilesContainer().getBasePath().length);
};
...
basebile.prototype.getOutputPath=function(){
    ...
    var basepath=util.path.getNormalizePath(this.getBilesContainer().getBasePath());
