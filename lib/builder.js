...
var builder = function (option) {
    ...
    option.rootPath = option.cwd ? option.cwd : process.cwd().replace(/\\/g, "/") + "/";
        ...
        (function (path) {
            ps.then(function () {
                return topolr.file(path).read().then(function (content) {
                    return content.replace(/topolr\.js[\?t=]*[0-9a-z]*/, function (str) {
                        var tp = require("path").resolve(ths._option.rootPath, "./node_modules/topolr/topolr.js");
                        return str.split("?")[0] + "?t=" + topolr.file(tp).hash();
                    }).replace(/topolr\.min\.js[\?t=]*[0-9a-z]*/, function (str) {
                        var tp = require("path").resolve(ths._option.rootPath, "./node_modules/topolr/topolr.min.js");
                        return str.split("?")[0] + "?t=" + topolr.file(tp).hash();
                    }).replace(/App\(\{[\w\W]*?\}\)/g, function (a) {
...
builder.prototype.getSourceMap = function () {
    var r = {};
    for (var i in config.sourceType) {
        r[i] = [];
    }
    return r;
};
